...
class Brain:
        ...
        try:
            with open(self.caminho_memoria, 'r', encoding='utf-8') as f:
                for linha in f.readlines()[-20:]:
                    d = json.loads(linha)
                    hist.append({"role": "model" if d["autor"] == "REGISTRO" else "user", "parts": [d["texto"]]})
        except: pass
        return hist
    ...
    def processar_entrada(self, prompt, tentativa=0):
        ...
        print(f"[REQ #{self.contador_requisicoes}] Tent. {tentativa+1}/2 | Chamadas: {len(self.chamadas_ultimo_minuto)}")

        try:
            res = self.chat.send_message(prompt)
            
            if res.candidates and (reason := res.candidates[0].finish_reason) != 1:
                msgs = {3: "Bloqueado por segurança", 2: "Muito longa. Reformule."}
                if reason in msgs:
