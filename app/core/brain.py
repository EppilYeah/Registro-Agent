...
class Brain:
    ...
    def _configurar_api_key(self):
        ...
        if hasattr(config, 'API_KEYS') and len(config.API_KEYS) > 1:
            ...
            config.API_KEY_ATUAL = (
                config.API_KEY_ATUAL + 1) % len(config.API_KEYS)
            ...
            print(
                f" [API] Rotação: Key #{config.API_KEY_ATUAL + 1}/{len(config.API_KEYS)} (Final: ...{final_key})")
        else:
    ...
    def processar_entrada(self, prompt_usuario, _tentativa_recursiva=0):
        ...
        if _tentativa_recursiva >= 3:
            print("[ERRO] Limite de tentativas atingido (3). Abortando.")
            return {
        ...
        try:
            ...
            print(
                f"[REQ #{self.contador_requisicoes}] Enviando prompt inicial")
            ...
            while response.parts and response.parts[0].function_call and turnos_funcao < 5:
                        ...
                        if hasattr(self, 'sistema') and self.sistema:
                            if hasattr(self.sistema, fname):
                                try:
                                    ...
                                    print(
                                        f"[TOOL] Resultado: {resultado_acao}")
                ...
                self.contador_requisicoes += 1
                print(
                    f"[REQ #{self.contador_requisicoes}] Devolvendo resultado da ferramenta")
            ...
            try:
                ...
                if not tentativa_correcao:
                    ...
                    self.contador_requisicoes += 1
                    print(
                        f"[REQ #{self.contador_requisicoes}] Corrigindo formato JSON")

                    try:
                        response = self.chat.send_message(
                            "Protocolo de Saída incorreto. Retorne APENAS o JSON final.")
                        texto_limpo = response.text.replace(
                            "```json", "").replace("```", "").strip()
                        dados_ia = json.loads(texto_limpo)
                    except:
            ...
            if not isinstance(dados_ia, dict):
                dados_ia = {"emocao": "neutro",
                            "texto_resposta": str(dados_ia)}

            texto_dados_ia = dados_ia.get(
                "texto_resposta", "Sem resposta vocal.")
            ...
            if "429" in str(e) or "quota" in str(e).lower():
                print(
                    f"[ERRO] Cota esgotada no modelo {self.modelo_atual_nome}")
                ...
                try:
                    ...
                    self.chat, self.modelo = self._carregar_modelo_seguro(
                        ignorar_modelos=ignorar)
                    print(
                        f"[INFO] Trocado para {self.modelo_atual_nome}. Tentando novamente...")
                    return self.processar_entrada(prompt_usuario, _tentativa_recursiva + 1)
                except Exception as retry_error:
                    print(
                        f"[ERRO] Todas as alternativas falharam: {retry_error}")
                    return {
    ...
    def _carregar_modelo_seguro(self, ignorar_modelos=None, _tentativa_key=0):
        ...
        for nome_modelo in config.LISTA_MODELOS:
            ...
            try:
                ...
                chat_pronto = modelo_teste.start_chat(
                    history=historico_completo)
        ...
        if modelos_testados >= len(config.LISTA_MODELOS):
            if hasattr(config, 'API_KEYS') and len(config.API_KEYS) > 1:
                print(
                    f"[INFO] Todos os modelos falharam com esta key. Tentando próxima...")
                self._configurar_api_key()
...
Saída: Está na hora de ingerir suas substâncias químicas. Espero que não se esqueça novamente.
        ...
        try:
            self.contador_requisicoes += 1
            print(
                f"[REQ #{self.contador_requisicoes}] Gerando texto de lembrete")
